   ...
   public function generate(Request $request)
{
    ...
    // Load JSON into an array (NOT a path string)
    $keyPath = storage_path('app/credentials/tts-key.json'); // adjust if yours lives elsewhere
    $keyData = json_decode(file_get_contents($keyPath), true);

    // If you EVER feed creds via .env JSON, normalize \n -> real newlines
    if (isset($keyData['private_key'])) {
        // convert any literal "\n" sequences to real newlines and normalize CRLF
        $keyData['private_key'] = str_replace(
            ["\r\n", "\r"],
            "\n",
            preg_replace('/\\\\n/', "\n", $keyData['private_key'])
        );
    ...
    $inputText = new SynthesisInput(['text' => $request->input('text')]);
    $voice     = new VoiceSelectionParams([
        'language_code' => $request->input('language'),
        'name'          => $request->input('voice'),
    ]);
    $audioCfg  = new AudioConfig(['audio_encoding' => AudioEncoding::MP3]);
    ...
    $fileName = ($request->input('audio_name') ?: 'tts-'.Str::random(10)).'.mp3';
